...
class GalleryViewModel(
    ...
    init {
        observeTimeline(source)
            .onEach { sections ->
                val available = sections.asSequence()
    ...
    override suspend fun reduce(intent: GalleryIntent) {
        when (intent) {
            ...
            is GalleryIntent.MediaLongPressed ->
                setState { copy(selection = selection.toggle(intent.item.id)) }
            ...
            is GalleryIntent.SectionTapped -> {
                ...
                setState {
                    val next =
                        if (alreadySelected) selection.selectedIds - ids.toSet()
            ...
            GalleryIntent.AddToAlbumRequested ->
